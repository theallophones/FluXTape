...
# --- Custom CSS for the Gradient Background and Component Styling ---
# NOTE: This block uses a standard string (not an f-string) to prevent
# Python from misinterpreting the CSS curly braces.
css = """
<style>
...
def file_to_data_url(path, mime="audio/mpeg"):
    ...
    try:
        ...
        st.error(f"Audio file not found: **{path}**. Please ensure it is in the same directory as this Python script on your GitHub repo.")
        return ""

audio_map = {k: file_to_data_url(v) for k, v in audio_files.items()}

# --- HTML/JS/CSS Block ---
# This uses a Python f-string to inject audio_map. JavaScript curly braces are escaped.
html = f"""
...
<script>
    ...
    function loadVersion(idx, keepTime=true) {{
        ...
        // Pause before loading new track for cleaner transition
        if (playing) ws.pause();
